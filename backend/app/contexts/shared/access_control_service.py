...
def get_accessible_group_ids(db: Session, user_id: int) -> list[int] | None:
    ...
    - 治理员(governor)：返回其绑定群组所属机构及下级机构的所有群组ID
    ...
    # 检查用户是否是治理员(governor)
    # governor角色绑定可能是 scope_type="group" + scope_id=群组ID
    # 也可能是 scope_type="agency" + scope_id=机构ID
    governor_role = next(
        (r for r in user_ctx["roles"] if r["role_code"] == "governor"),
        None,
    )
    ...
    if governor_role:
        ...
        if governor_role["scope_type"] == "group" and governor_role["scope_id"]:
            # 治理员绑定的是群组，找到该群组的牵头机构
            bound_group = db.query(GroupInfo).filter(
                GroupInfo.id == governor_role["scope_id"]
            ).first()
            if bound_group and bound_group.lead_agency_id:
                # 获取该机构及所有下级机构的群组
                agency_ids = _get_all_sub_agency_ids(db, bound_group.lead_agency_id)
                # 通过GroupMember查找这些机构参与的所有群组
                ...
                accessible_group_ids.update(g[0] for g in member_group_ids)
            # 也加入直接绑定的群组
            accessible_group_ids.add(governor_role["scope_id"])
        
        elif governor_role["scope_type"] == "agency" and governor_role["scope_id"]:
            # 治理员绑定的是机构
            agency_ids = _get_all_sub_agency_ids(db, governor_role["scope_id"])
            from app.models.group import GroupMember
            member_group_ids = db.query(GroupMember.group_id).filter(
                GroupMember.agency_id.in_(agency_ids),
                GroupMember.join_status == "active",
        ...
        if accessible_group_ids:
            return list(accessible_group_ids)
        return []
...
def _get_all_sub_agency_ids(db: Session, agency_id: int) -> list[int]:
    """递归获取机构及其所有下级机构ID"""
    ...
    for child in children:
        ...
        if child_id not in agency_ids:
            ...
            agency_ids.extend(_get_all_sub_agency_ids(db, child_id))
    return agency_ids
